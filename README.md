...
- To open the panel: run the command **GitX: Open Panel** from the Command Palette or click the **GitX** status bar
  item.
- For local UI development, go to `src/webview` and run the dev server (`pnpm dev`) to iterate with hot reload. For
  packaged runs, build the Vue app (`pnpm build`) and ensure the output is placed under `dist/` so the extension can
